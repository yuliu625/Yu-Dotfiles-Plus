...
我知道[PEP8](https://peps.python.org/pep-0008/)中关于包导入的规范，并且也知道经典工具例如`isort`、`black`、`ruff`、`flake8`这样的自动化工具。我明白按照如内置、字母顺序在多人大型长期维护项目的好处，以及约定大于配置的方便。
...
- 外部包。先第三方包，然后标准库包。
- 类型。使用TYPE_CHECKING减少导入开销。仅type-hints相关包。
```python
"""
